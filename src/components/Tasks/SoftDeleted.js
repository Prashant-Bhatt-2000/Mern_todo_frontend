...
const SoftDeleted = ({ search }) => {
  ...
  useEffect(() => {
    async function fetchTodos() {
      try {
        const response = await fetch(`/getsoftDeleted`, {
          method: "GET",
          headers: {
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-10 ml-16">
    {todos.length > 0 ? (
      todos.map((todo, index) => (
        <div key={index} className="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-300 dark:border-gray-700">
          ...
          <div className="flex">
            ...
            <RestoreTodo onRestore={handlerestore} todo={todo._id}/>
          </div>
        </div>
        ))
        ) : (
        <div class="flex items-center justify-center h-screen">
          <div class="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-300 dark:border-gray-700">
            <h5 class="mb-2 text-2xl font-semibold tracking-tight text-gray-900 dark:text-gray-800">No Soft Deleted Task</h5>
